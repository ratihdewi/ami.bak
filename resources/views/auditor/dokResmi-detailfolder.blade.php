...
@section('linking')
    <a href="/dokumenresmiAMI-auditor-folderall" class="mx-1">
        Dokumen Resmi AMI
    </a>/
...
<div class="container pt-5" style="min-height: 100vh">
    ...
    @endif
    <div class="buton-addFile" id="button-addFile">
          ...
          @foreach ($files as $files)
            <tr>
              ...
              <td class="text-center">
                <button type="button" id="editmodalfolder" class="btn btn-primary btn-sm p-0"data-bs-toggle="modal" data-bs-target="#editFile" onclick="editmodal({{ $files->id }})" title="Rename"><i class="bi bi-pencil-square text-white mx-1 my-2 h5"></i></button>
                ...
                <a href="/dokumenresmiAMI-spm-folderdetail-delete/{{ $files->id }}" onclick="return confirm('Apakah Anda yakin akan menghapus folder {{ $files->fileName }} ?')"><button class="btn btn-danger btn-sm p-0" title="Hapus"><i class="bi bi-trash-fill text-white mx-1 my-2 h5"></i></button></a>
              </td>
            </tr>
          @endforeach
        </tbody>
