...
@section('container')
  <div class="container">
      <div class="d-flex my-4 justify-content-between">
        @foreach ($auditee_ as $auditee)
          <a href="/auditor-auditeeBA/{{ $auditee->id }}/{{ $auditee->tahunperiode }}"><button class="btn btn-secondary btn-sm me-2 fw-semibold" type="button">Kembali</button></a>
          @endforeach
        <div class="btn-right">
          @foreach ($auditee_ as $auditee)
            ...
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                      ...
                      <td class="col-2 text-center">
                        <a href="/auditor-BA-editpeluangpeningkatan/{{ $peningkatan->id }}" class="mx-2"><button class="bg-primary border-0 rounded-1"><i class="bi bi-pencil-square text-white" title="Edit"></i></button></a>
                        <a href="/BA-deletepeluangpeningkatan/{{ $peningkatan->id }}" class="mx-2"><button class="bg-danger border-0 rounded-1"><i class="bi bi-trash text-white" title="Hapus" onclick="return confirm('Apakah Anda yakin akan menghapus data peluang peningkatan ini?')"></i></button></a>
                      </td>
                    </tr>
                    @endforeach
                </tbody>
                        ...
                        @else
                            @foreach ($ba_ami->get() as $ba)
                              ...
                              @else
                                ...
                                @endforeach
                                  <button class="border-0 bi bi-pen" type="button" onclick="return confirm('Apakah Anda yakin akan mengajukan persetujuan atau menyetujui Audit Lapangan ini?')"
                                  @if ((Auth::user()->name != $auditee->ketua_auditor) || count($auditee->doklampiran()->get()) == 0 || count($pelpeningkatan_) == 0 || count($daftarhadir_) == 0 || $ba_ami->doesntExist())
                                      {{ "disabled" }}
                                  @endif
                                  style="background: none"></button>
                                </a>  
