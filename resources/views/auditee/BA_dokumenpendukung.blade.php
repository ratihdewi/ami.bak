...
@section('linking')
    ...
    <a href="/auditee-auditeeBA/{{ $beritaacara_->auditee_id }}/{{ $beritaacara_->tahunperiode }}" class="mx-1">
    {{ $beritaacara_->auditee->unit_kerja }}
    </a>/

    <a href="/auditee-BA-AMI/{{ $beritaacara_->auditee_id }}/{{ $beritaacara_->tahunperiode }}" class="mx-1">
    BA - AMI
    </a>/

    <a href="/auditee-BA-dokumenpendukung/{{ $beritaacara_->auditee_id }}" class="mx-1">
    Dokumen Pendukung
    </a>/
...
@section('container')
  <div class="container vh-100 mb-4">
    ...
    <form id="myForm" action="/BA-storedokumenpendukung/{{ $beritaacara_->auditee_id }}" method="POST" enctype="multipart/form-data">
      ...
      @endif
      {{-- Dokumen Pendukung --}}
      <div class="row sectionName mx-0 mb-2 mt-2">
        ...
        <tbody>
            ...
            <tr>
              ...
              <td class="col-2 text-center">
                <a href="/BA-deletedokumenpendukung/{{ $dokumenpendukung->id }}" class="mx-2" onclick="return confirm('Apakah Anda yakin akan menghapus dokumen {{ $dokumenpendukung->namaDokumen }} ?')"><button class="btn btn-danger btn-sm" title="Hapus"><i class="bi bi-trash text-white h7"></i></button></a>
              </td>
            </tr>
            @endforeach
        </tbody>
